...
namespace cxsw
{
    ...
    bool cxLoadGCode(const QString& fileName, QStringList& layerString, QStringList& prefixString, QStringList& tailCode, ccglobal::Tracer* tracer)
    {
        ...
        if (!file.open(QIODevice::ReadOnly) || (fileSize == 0))
        {
        ...
        while (!file.atEnd())
        {
            ...
            if (temp.contains(";TIME_ELAPSED:"))
            {
                layerString.append(value);
                value.clear();
                continue;
            }
        }
        tailCode.append(value);
        file.close();
        return true;
	}
